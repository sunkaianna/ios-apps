...
#warning Here is where you draw 2D objects
    ...
    //Draw lines
        ...
        [square drawRectangle:context :CGPointMake((_contextSize.y/_viewSize.x), (_contextSize.x/_viewSize.y)) angle: _angle translateOrRotate:_buttonState incrementVal:_translateIncrement];
...
- (IBAction)objectRotatedAction:(id)sender {
    //NSLog(@"%f", self.objectRotatedValue.value);
    _angle  = self.objectRotatedValue.value;
}

- (IBAction)rotateOrTranslateControl:(id)sender {
    if(self.rotateOrTranslateButton.on)
        _buttonState = YES;
    else
        _buttonState = NO;
}
...
- (IBAction)translateScalingController:(id)sender {
    if(_buttonState == NO)
    _translateIncrement = self.translateIncrementValue.value;
}
